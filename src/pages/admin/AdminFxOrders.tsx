...
const getCustomerName = (order: AdminFxOrder): string => {
  const directName = order.customer_snapshot?.user?.full_name || order.user?.full_name;
...
const AdminFxOrders = () => {
  ...
  const confirmMutation = useMutation({
    ...
    onError: (error) => {
      const message = `Unable to confirm this FX order. Error category: ${getAdminErrorCategory(error instanceof Error ? error.message : null)}.`;
  ...
  const rejectMutation = useMutation({
    ...
    onError: (error) => {
      const message = `Unable to reject this FX order. Error category: ${getAdminErrorCategory(error instanceof Error ? error.message : null)}.`;
      setActionError(message);
