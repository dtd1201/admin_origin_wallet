...
const AdminBeneficiaries = () => {
  ...
  return <div className="px-4 py-5 sm:px-6 lg:px-10 lg:py-8"><div className="grid gap-6 2xl:grid-cols-[minmax(0,1.7fr)_minmax(340px,0.7fr)]">
    <Card className="rounded-[28px] border-0 shadow-[0_18px_60px_rgba(15,23,42,0.08)]"><CardHeader><CardTitle className="flex items-center gap-2 text-2xl"><UserRoundCheck className="h-6 w-6 text-emerald-600" />Beneficiaries</CardTitle><CardDescription>Read-only payout and beneficiary banking details.</CardDescription></CardHeader><CardContent className="space-y-4">
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200"><div className="overflow-x-auto"><Table className="min-w-[700px] table-fixed 2xl:min-w-0"><TableHeader><TableRow><TableHead className="w-[20%]">Beneficiary reference</TableHead><TableHead className="w-[22%]">Name</TableHead><TableHead className="w-[12%]">Currency</TableHead><TableHead className="w-[16%]">Payout status</TableHead><TableHead className="w-[16%]">Provider mapping</TableHead><TableHead className="w-[14%] text-right">Detail</TableHead></TableRow></TableHeader><TableBody>
        {rows.length ? rows.map((row) => <TableRow key={row.id}><TableCell className="font-semibold">{String(row.id)}</TableCell><TableCell>{displayName(row.company_name || row.full_name)}</TableCell><TableCell>{row.currency || "Unknown"}</TableCell><TableCell><Badge variant="secondary">{row.status || "Unknown"}</Badge></TableCell><TableCell>{mappingStatus(row.external_beneficiary_id)}</TableCell><TableCell className="text-right"><Button type="button" size="sm" variant="outline" onClick={() => setSelectedId(row.id)}><Eye className="h-4 w-4" />Inspect</Button></TableCell></TableRow>) : <TableRow><TableCell colSpan={6} className="h-36 text-center text-slate-500">{listQuery.isLoading ? "Loading beneficiaries..." : "No beneficiaries found."}</TableCell></TableRow>}
      </TableBody></Table></div></div>
      <Pagination page={page} lastPage={lastPage} onPage={setPage} />
    </CardContent></Card>
    <Card className="rounded-[28px] border-0 shadow-[0_18px_60px_rgba(15,23,42,0.08)]"><CardHeader><CardTitle>Beneficiary detail</CardTitle><CardDescription>Provider identifiers remain hidden. Beneficiary banking details are available for operations.</CardDescription></CardHeader><CardContent>
      {detailQuery.isError ? <ErrorPanel message={detailQuery.error instanceof Error ? detailQuery.error.message : "Unable to load beneficiary detail."} /> : detailQuery.data ? <div className="grid gap-3 md:grid-cols-2 2xl:grid-cols-1"><Detail label="Beneficiary reference" value={String(detailQuery.data.id)} /><Detail label="Name" value={displayName(detailQuery.data.company_name || detailQuery.data.full_name)} /><Detail label="Currency" value={detailQuery.data.currency || "Unknown"} /><Detail label="Bank name" value={detailQuery.data.bank_name || "-"} /><Detail label="Account number" value={detailQuery.data.account_number || "-"} /><Detail label="IBAN" value={detailQuery.data.iban || "-"} /><Detail label="SWIFT / BIC" value={detailQuery.data.swift_bic || "-"} /><Detail label="Bank code" value={detailQuery.data.bank_code || "-"} /><Detail label="Branch code" value={detailQuery.data.branch_code || "-"} /><Detail label="Payout status" value={detailQuery.data.status || "Unknown"} /><Detail label="Provider association" value={String(detailQuery.data.provider_id)} /><Detail label="Provider mapping" value={mappingStatus(detailQuery.data.external_beneficiary_id)} /><Detail label="Created" value={formatDate(detailQuery.data.created_at)} /><Detail label="Updated" value={formatDate(detailQuery.data.updated_at)} /></div> : <EmptyDetail text={detailQuery.isLoading ? "Loading beneficiary detail..." : "Select a beneficiary to inspect safe fields."} />}
    </CardContent></Card>
  </div></div>;
};
