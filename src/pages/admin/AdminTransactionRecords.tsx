...
const AdminTransactionRecords = () => {
  ...
  return <div className="px-4 py-5 sm:px-6 lg:px-10 lg:py-8">
    <div className="grid gap-6 [min-width:1800px]:grid-cols-[minmax(0,1.8fr)_minmax(360px,0.65fr)]">
      <Card className="rounded-[28px] border-0 shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="overflow-hidden rounded-3xl border border-slate-200"><div className="overflow-x-auto"><Table className="min-w-[900px] table-fixed [min-width:1800px]:min-w-0"><TableHeader><TableRow><TableHead className="w-[16%]">Record</TableHead><TableHead className="w-[18%]">Type</TableHead><TableHead className="w-[14%]">Direction</TableHead><TableHead className="w-[12%]">Amount</TableHead><TableHead className="w-[9%]">Fee</TableHead><TableHead className="w-[10%]">Status</TableHead><TableHead className="w-[13%]">Booked</TableHead><TableHead className="w-[8%] text-right">Detail</TableHead></TableRow></TableHeader><TableBody>
            {rows.length ? rows.map((record) => <TableRow key={record.id}><TableCell><div className="break-all font-semibold leading-5 text-slate-900">{displayIdentifier(record.external_transaction_id)}</div><div className="text-xs text-slate-400">Record {displayIdentifier(record.id)}</div></TableCell><TableCell className="break-words text-xs leading-5 [overflow-wrap:anywhere]">{record.transaction_type || "-"}</TableCell><TableCell className="break-words text-xs leading-5 [overflow-wrap:anywhere]">{record.direction || "-"}</TableCell><TableCell className="whitespace-nowrap text-sm">{formatAmount(record.amount, record.currency)}</TableCell><TableCell className="whitespace-nowrap text-sm">{formatAmount(record.fee_amount, record.currency)}</TableCell><TableCell><Badge className={statusClassName(record.status)}>{record.status || "unknown"}</Badge></TableCell><TableCell className="text-xs leading-5">{formatDate(record.booked_at)}</TableCell><TableCell className="text-right"><Button type="button" size="sm" variant="outline" onClick={() => setSelectedId(record.id)}><Eye className="h-4 w-4" />Inspect</Button></TableCell></TableRow>) : <TableRow><TableCell colSpan={8} className="h-36 text-center text-slate-500">{recordsQuery.isLoading ? "Loading transaction records..." : "No transaction records found."}</TableCell></TableRow>}
