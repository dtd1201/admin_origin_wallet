...
const displayIdentifier = (value?: string | number | null) => {
  ...
  return <div className="space-y-6 px-4 py-5 sm:px-6 lg:px-10 lg:py-8">
    <Card className="rounded-[28px] border-0 shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200"><div className="overflow-x-auto"><Table className="min-w-[820px] table-fixed xl:min-w-0"><TableHeader><TableRow><TableHead className="w-[19%]">Wallet</TableHead><TableHead className="w-[18%]">User</TableHead><TableHead className="w-[8%]">Currency</TableHead><TableHead className="w-[12%]">Available balance</TableHead><TableHead className="w-[12%]">Ledger balance</TableHead><TableHead className="w-[12%]">Reserved balance</TableHead><TableHead className="w-[8%]">Status</TableHead><TableHead className="w-[11%] text-right">Detail</TableHead></TableRow></TableHeader><TableBody>{wallets.length ? wallets.map((wallet) => <TableRow key={wallet.id}><TableCell className="break-all font-semibold leading-5">{displayIdentifier(wallet.account_reference)}</TableCell><TableCell className="break-all text-sm leading-5">{wallet.user?.email || "-"}</TableCell><TableCell>{wallet.currency || "-"}</TableCell><TableCell>{formatAmount(wallet.available_balance, wallet.currency)}</TableCell><TableCell>{formatAmount(wallet.ledger_balance, wallet.currency)}</TableCell><TableCell>{formatAmount(wallet.hold_balance, wallet.currency)}</TableCell><TableCell><Badge className={statusClassName(safeWalletStatus(wallet.status))}>{safeWalletStatus(wallet.status)}</Badge></TableCell><TableCell className="text-right"><Button type="button" size="sm" variant="outline" onClick={() => setSelectedWalletId(wallet.id)}><Eye className="h-4 w-4" />Inspect wallet</Button></TableCell></TableRow>) : <TableRow><TableCell colSpan={8} className="h-32 text-center text-slate-500">{walletsQuery.isLoading ? "Loading wallet accounts..." : "No wallet accounts found."}</TableCell></TableRow>}</TableBody></Table></div></div>
    ...
    <Card className="rounded-[28px] border-0 shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200"><div className="overflow-x-auto"><Table className="min-w-[820px] table-fixed xl:min-w-0"><TableHeader><TableRow><TableHead className="w-[17%]">Reference</TableHead><TableHead className="w-[15%]">Wallet</TableHead><TableHead className="w-[12%]">Type</TableHead><TableHead className="w-[12%]">Amount</TableHead><TableHead className="w-[12%]">Balance after</TableHead><TableHead className="w-[8%]">Status</TableHead><TableHead className="w-[12%]">Posted</TableHead><TableHead className="w-[12%] text-right">Detail</TableHead></TableRow></TableHeader><TableBody>{entries.length ? entries.map((entry) => <TableRow key={entry.id}><TableCell className="break-all font-semibold leading-5">{displayIdentifier(entry.reference)}</TableCell><TableCell className="break-all text-sm leading-5">{displayIdentifier(entry.wallet?.account_reference)}</TableCell><TableCell className="break-words text-sm leading-5">{entry.entry_type}</TableCell><TableCell>{formatAmount(entry.amount, entry.currency)}</TableCell><TableCell>{formatAmount(entry.balance_after, entry.currency)}</TableCell><TableCell><Badge className={statusClassName(entry.status)}>{entry.status}</Badge></TableCell><TableCell>{formatDate(entry.posted_at || entry.created_at)}</TableCell><TableCell className="text-right"><Button type="button" size="sm" variant="outline" onClick={() => setSelectedLedgerId(entry.id)}><Eye className="h-4 w-4" />Inspect entry</Button></TableCell></TableRow>) : <TableRow><TableCell colSpan={8} className="h-32 text-center text-slate-500">{ledgerEntriesQuery.isLoading ? "Loading ledger entries..." : "No ledger entries found."}</TableCell></TableRow>}</TableBody></Table></div></div>
...
const Detail = ({ label, value }: { label: string; value: string }) => <div className="rounded-2xl border border-slate-200 p-3"><div className="text-xs uppercase tracking-[0.14em] text-slate-400">{label}</div><div className="mt-1 break-words font-medium text-slate-900 [overflow-wrap:anywhere]">{value}</div></div>;
